<?php /*a:1:{s:70:"E:\sjx\PHPTutorial\WWW\tp5.1demo\application\admin\view\mall\spec.html";i:1535868708;}*/ ?>
                        ...
                        <thead>
                         ...
                         <tbody>
                         	...
                         	<tr rel="<?php echo htmlentities($vo['id']); ?>">
                                 ...
                                 <td><?php if(is_array($vo['item']) || $vo['item'] instanceof \think\Collection || $vo['item'] instanceof \think\Paginator): $i = 0; $__LIST__ = $vo['item'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vv): $mod = ($i % 2 );++$i;?><?php echo htmlentities($vv); ?>，<?php endforeach; endif; else: echo "" ;endif; ?></td>
...
function add(){
	layer_show("添加","<?php echo url('mall/spec_add'); ?>",800,450);
}

function edit(){
	var id = getid();
	if(id>0){
		layer_show("编辑","<?php echo url('mall/spec_add'); ?>?id="+id,800,450);
	}else{
		layer_alert('请选择目标');	
	}
}
...
function del(){
	...
	if(ids != ''){
		parent.layer.confirm('确认要删除吗？',{icon: 3, title:'警告'},function(index){
			$.getJSON("<?php echo url('mall/spec_del'); ?>",{'ids':ids},function(res){
				if(res.code == 1){
