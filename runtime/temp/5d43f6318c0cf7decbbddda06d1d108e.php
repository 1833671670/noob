<?php /*a:1:{s:97:"D:\phpstudy\PHPTutorial\WWW\noob\sjx20190429\tp5.1demo\application\admin\view\infor\art_type.html";i:1535628188;}*/ ?>
...
function add(){
	layer_show("添加","<?php echo url('infor/art_type_add'); ?>",800,360);
}
...
function edit(){
	...
	if(id>0)
	{
		layer_show("编辑","<?php echo url('infor/art_type_add'); ?>?id="+id,800,360);
	}else{
		layer_alert('请选择目标');	
	}
}
...
function del(){
	...
	if(id>0){
		parent.layer.confirm('确认要删除吗？',{icon: 3, title:'警告'},function(index){
			$.getJSON("<?php echo url('infor/art_type_del'); ?>",{'id':id},function(res){
				if(res.code == 1){
