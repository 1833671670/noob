<?php /*a:1:{s:75:"E:\sjx\PHPTutorial\WWW\tp5.1demo\application\admin\view\infor\adv_type.html";i:1535627983;}*/ ?>
...
function add(){
	layer_show("添加","<?php echo url('infor/adv_type_add'); ?>",800,320);
}

function edit(){
	var id = getid();
	if(id>0){
		layer_show("编辑","<?php echo url('infor/adv_type_add'); ?>?id="+id,800,320);
	}else{
		layer_alert('请选择目标');	
	}
}
...
function del(){
	...
	if(ids != ''){
		parent.layer.confirm('确认要删除吗？',{icon: 3, title:'警告'},function(index){
			$.getJSON("<?php echo url('infor/adv_type_del'); ?>",{'ids':ids},function(res){
				if(res.code == 1){
