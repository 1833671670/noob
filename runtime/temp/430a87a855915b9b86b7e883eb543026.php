<?php /*a:1:{s:70:"E:\sjx\PHPTutorial\WWW\tp5.1demo\application\admin\view\infor\adv.html";i:1536221923;}*/ ?>
    ...
    <div class="ibox float-e-margins">
        ...
        <div class="ibox-content">
            <div class="row">
                ...
                <div class="col-sm-7">                               
                	<form name="admin_list_sea" action="<?php echo url('infor/adv'); ?>" class="form-search form-inline" method="post" >
                        <div class="input-group">
                            <div class="form-group">
                                <select class="form-control m-b" name="type_id">
                                     <option value="">请选择分类</option>
                                     <?php if(is_array($art_type) || $art_type instanceof \think\Collection || $art_type instanceof \think\Paginator): $i = 0; $__LIST__ = $art_type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                                     <option <?php if(!(empty($search['type_id']) || (($search['type_id'] instanceof \think\Collection || $search['type_id'] instanceof \think\Paginator ) && $search['type_id']->isEmpty()))): if($vo['id'] == $search['type_id']): ?>selected<?php endif; endif; ?> value="<?php echo htmlentities($vo['id']); ?>"><?php echo htmlentities($vo['name']); ?></option>
                        ...
                        <thead>
                         ...
                         <tbody>
                         	...
                         	<tr rel="<?php echo htmlentities($vo['id']); ?>">
                                 ...
                                 <td><?php echo htmlentities($vo['ty_name']); ?></td>
                                 <td><?php echo htmlentities($vo['url']); ?></td>
                                 <td> <?php if($vo['is_open'] == 0): ?>
...
function add(){
	layer_show("添加","<?php echo url('infor/adv_add'); ?>",800,550);
}

function edit(){
	var id = getid();
	if(id>0){
		layer_show("编辑","<?php echo url('infor/adv_add'); ?>?id="+id,800,550);
	}else{
		layer_alert('请选择目标');	
	}
}
...
function del(){
	...
	if(ids != ''){
		parent.layer.confirm('确认要删除吗？',{icon: 3, title:'警告'},function(index){
			$.getJSON("<?php echo url('infor/adv_del'); ?>",{'ids':ids},function(res){
				if(res.code == 1){
	...
	var ids = getids();
 	if(ids != ''){
 		parent.layer.confirm('确认要操作吗？',{icon: 3, title:'警告'},function(index){
 			$.ajax({
 				type: 'POST',
 				url: "<?php echo url('infor/adv_act'); ?>?flag="+flag+"&ids="+ids,
 				dataType: 'json',
