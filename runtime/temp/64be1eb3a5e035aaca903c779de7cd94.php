<?php /*a:1:{s:98:"D:\phpstudy\PHPTutorial\WWW\noob\sjx20190429\tp5.1demo\application\admin\view\mall\goodsmodel.html";i:1535709597;}*/ ?>
                        ...
                        <thead>
                         ...
                         <tbody>
                         	...
                         	<tr rel="<?php echo htmlentities($vo['id']); ?>">
                                 ...
                                 <td><?php echo htmlentities($vo['attr']); ?></td>
...
function add(){
	layer_show("添加","<?php echo url('mall/goodsmodel_add'); ?>",800,380);
}

function edit(){
	var id = getid();
	if(id>0){
		layer_show("编辑","<?php echo url('mall/goodsmodel_add'); ?>?id="+id,800,380);
	}else{
		layer_alert('请选择目标');	
	}
}
...
function del(){
	...
	if(ids != ''){
		parent.layer.confirm('确认要删除吗？',{icon: 3, title:'警告'},function(index){
			$.getJSON("<?php echo url('mall/goodsmodel_del'); ?>",{'ids':ids},function(res){
				if(res.code == 1){
