<?php /*a:1:{s:97:"D:\phpstudy\PHPTutorial\WWW\noob\sjx20190429\tp5.1demo\application\admin\view\config\message.html";i:1537008608;}*/ ?>
        ...
        <div class="ibox-content">
            <div class="row">
                ...
                <div class="col-sm-7 pull-right">                               
                	<form name="admin_list_sea" action="<?php echo url('infor/art'); ?>" class="form-search form-inline" method="post" >
...
function add(){
	var index = layer.open({
		type: 2,
		title: '添加文章',
		content: "<?php echo url('infor/art_add'); ?>"
	});
	layer.full(index);
}
...
function edit(){
	...
	if(id>0){
		var index = layer.open({
			type: 2,
			title: '编辑文章',
			content: '<?php echo url("infor/art_add"); ?>?id='+id
		});
		layer.full(index);
	}else{
		layer_alert('请选择目标');	
...
function del(){
	...
	if(ids != ''){
		parent.layer.confirm('确认要删除吗？',{icon: 3, title:'警告'},function(index){
			$.getJSON("<?php echo url('infor/art_del'); ?>",{'ids':ids},function(res){
				if(res.code == 1){
	...
	var ids = getids();
 	if(ids != ''){
 		parent.layer.confirm('确认要操作吗？',{icon: 3, title:'警告'},function(index){
 			$.ajax({
 				type: 'POST',
 				url: "<?php echo url('infor/art_act'); ?>?flag="+flag+"&ids="+ids,
 				dataType: 'json',
